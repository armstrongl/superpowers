---
description: Use before any creative work — creating features, building components, adding functionality, or modifying behavior — before any implementation begins.
name: brainstorming
---
...
## Anti-Pattern: "This is too small to need a design"

Every project goes through this process. A todo list, a single-function utility,
a config change — all of them. Small projects are where unexamined assumptions
cause the most wasted work. The design can be short (a few sentences for truly
small projects), but you MUST present it and get approval.
...
6. **Write design doc** — save to `docs/supapowers/specs/YYYY-MM-DD-<topic>-design.md`
   and commit
7. **Spec review loop** — dispatch spec-document-reviewer subagent with precisely
   crafted review context (never your session history); fix issues and
   re-dispatch until approved (max 5 iterations, then surface to human)
...
frontend-design, mcp-builder, or any other implementation skill. The ONLY
skill you invoke after brainstorming is writing-plans.
...
1. Dispatch spec-document-reviewer subagent (see `agents/spec-document-reviewer.md`)
...
| File | Purpose |
| ------ | --------- |
| `references/visual-companion.md` | Full visual companion guide: server usage, HTML authoring, CSS classes, event format |
| `references/fetch-references.md` | List of authoritative reference sources for this skill |
...
| File | Purpose |
| ------ | --------- |
| `scripts/start-server.sh` | Start the visual companion server |
| `scripts/stop-server.sh` | Stop the visual companion server |
| `scripts/server.js` | Visual companion Node.js server |
| `scripts/helper.js` | Client-side helper script injected into served pages |
| `scripts/fetch_resources.py` | Fetch fresh reference content from authoritative sources |

## Asset index

| File | Purpose |
| ------ | --------- |
| `assets/frame-template.html` | HTML frame template: CSS theme, selection infrastructure, mock elements |
